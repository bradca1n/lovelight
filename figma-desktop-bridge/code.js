...
// ============================================================================
// WRITE QUEUE — serializes all write operations across concurrent agents.
// Saves and restores figma.currentPage around each operation so each agent's
// commands run on their declared page, regardless of which page is active.
// Read-only operations bypass the queue entirely.
// ============================================================================
var _writeQueue = Promise.resolve();

function enqueueWrite(pageId, fn) {
  var result = _writeQueue.then(async function() {
    var prev = figma.currentPage;
    if (pageId) {
      await figma.loadAllPagesAsync();
      var target = figma.root.children.find(function(c) { return c.id === pageId; });
      if (target) figma.currentPage = target;
    }
    try {
      return await fn();
    } finally {
      figma.currentPage = prev; // always restore, even on error or early return
    }
  });
  _writeQueue = result.catch(function() {}); // keep queue alive after errors
  return result;
}
...
// Read-only operations that bypass the write queue — no document mutations,
// no page context dependency. All other operations are serialized.
var READ_ONLY_OPS = {
  GET_COMPONENT: true,
...
figma.ui.onmessage = function(msg) {
  ...
  if (READ_ONLY_OPS[msg.type]) {
    run();
  } else {
    enqueueWrite(msg.pageId, run);
  }
};
